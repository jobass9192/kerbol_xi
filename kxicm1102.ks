...
until runmode = -1
{
	...
	if runmode = 14
	{
		...
		{
			set current_action to "Circularizing".
			set node1 to circularize_noinc("ap").
	...
	{
		set circularized to execute_node_s2(node1, run, canexecute).
		set run to 1.
		if circularized = true
		{
			set output to "".
			remove node1.
			set current_action to "Circularization Complete".
			set p_seek to 999.
			set y_seek to 999.
			set r_seek to 999.
			set runmode to 16.
