...
until runmode = -1
{
	...
	if runmode = 1
	{
		if (timelaunch - time:seconds) <= 102
		{
			set current_action to "Controls Check".
			set runmode to 2.
			clearscreen.
		}
	}
	if runmode = 2 //controls check
	{
		if (timelaunch - time:seconds) > 100
		{
			set ship:control:pitch to 1.
			set ship:control:yaw to 0.
			set ship:control:roll to 0.
		}
		else if (timelaunch - time:seconds) > 98
		{
			set ship:control:pitch to -1.
			set ship:control:yaw to 0.
			set ship:control:roll to 0.
		}
		else if (timelaunch - time:seconds) > 96
		{
			set ship:control:pitch to 0.
			set ship:control:yaw to 1.
			set ship:control:roll to 0.
		}
		else if (timelaunch - time:seconds) > 94
		{
			set ship:control:pitch to 0.
			set ship:control:yaw to -1.
			set ship:control:roll to 0.
		}
		else if (timelaunch - time:seconds) > 92
		{
			set ship:control:pitch to 0.
			set ship:control:yaw to 0.
			set ship:control:roll to 1.
		}
		else if (timelaunch - time:seconds) > 90
		{
			set ship:control:pitch to 0.
			set ship:control:yaw to 0.
			set ship:control:roll to -1.
	...
	else if runmode = 3 //waiting step where crew follows appropriate checklists
	{
		
		if (timelaunch - time:seconds) <= 30
		{
			set timelaunch to time:seconds + 30.
			set current_action to "Holding Count Until Crew Action".
		}
		else
		{
